...
```{r}
ggplot(data = demand_df, aes(x = demand, y = mc_predict, color = district)) + 
  geom_line() +
  theme_minimal() +
...
district_costs_a <- tibble(kern_cost, mojave_cost, antelope_cost, ventura_cost) 
district_costs %>% 
...
ggplot(data = demand_df, aes(x = demand, y = mc_predict, color = district)) + 
  ...
  theme_minimal() +
  labs(title = "Predicted Marginal Demand Curves per District", 
       x = "Water Demand (AF)", 
       y = "$/AF",
       color = "District Name")
